...
public class POIUtils {
            ...
            if (sheet != null) {
                ...
                for (int i = 0; i < titleName.length; i++) {
                    ...
                    if (!getCellValue(titleRow.getCell(i)).equals(titleName[i])) {
                        throw new SuperBootException("标题有误！顺序应为【" + FastJsonUtils.toJSONFeatures(titleName) + "】");
                    }
                }
                //循环除了第一行的所有行  
    ...
    private static void checkFile(MultipartFile file) throws IOException {
        ...
        if (StringUtils.isEmpty(fileName)) {
            throw new SuperBootException("名称为空");
        }
        //判断文件是否是excel文件  
    ...
    public static Workbook toExcel(String[] titleName, List<Object[]> resultList) {
        //创建excel工作簿
